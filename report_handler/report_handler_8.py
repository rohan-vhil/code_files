...
class LocalStorage:
    ...
    def save_device_data(self, device_id, data_dict):
        ...
        self.buffer.append((capture_time, device_id, json.dumps(clean_data)))
...
class dataBank:
    ...
    def calculate_missing_power(self, data):
        if "L1_power" not in data and "L1_voltage" in data and "L1_current" in data:
            data["L1_power"] = round(data["L1_voltage"] * data["L1_current"], 2)
        
        if "L2_power" not in data and "L2_voltage" in data and "L2_current" in data:
            data["L2_power"] = round(data["L2_voltage"] * data["L2_current"], 2)
            
        if "L3_power" not in data and "L3_voltage" in data and "L3_current" in data:
            data["L3_power"] = round(data["L3_voltage"] * data["L3_current"], 2)
            
        if "total_power" not in data and "voltage" in data and "current" in data:
    ...
    def getAvg(self, device_id):
        ...
        if len(device_data) > 0:
            ...
            self.avg_data[str(device_id)] = self.calculate_missing_power(self.avg_data[str(device_id)])
                    ...
                    if response.status_code == 200:
                        for entry in unsent_data:
                            for key, value in entry.items():
                                if key.isdigit():
                                    self.storage.save_device_data(int(key), value)
                        self.storage.flush()
                        self._clear_unsent_data()
                except requests.RequestException as e:
                    logging.error(f"Failed to send stored data: {e}")
