...
class dataBank:
    ...
    def calculate_missing_power(self, data):
        ...
        if not hasattr(self, "_cumulative_input_energy"):
            self._cumulative_input_energy = 0.0

        if "voltage" in data and "current" in data and "total_power" not in data:
            data["total_power"] = round(data["voltage"] * data["current"], 2)

        if "input_power" in data and "input_energy" not in data:
            current_time = time.time()
            dt = current_time - self._last_energy_time
            power_kw = data["input_power"] / 1000 if data["input_power"] > 1000 else data["input_power"]
            energy_increment = power_kw * (dt / 3600)
            self._cumulative_input_energy += energy_increment
            data["input_energy"] = round(self._cumulative_input_energy, 2)
