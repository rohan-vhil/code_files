...
class modbusTCPDevice(ctrl.systemDevice):
    ...
    def hard_reset(self):
        print(f"---- HARD RESET triggered for TCP device {self.modbusTCP_comm_details.ip} to clear stale buffers ----")
        try:
            self.mbus_client.close()
        except Exception:
            pass
    ...
    def writeDataToRegisters(self, reg_data_list,addr):
        print("-------into writeDataToRegisters in ModbusTCPDevice------")
        try:
            if not self.mbus_client.is_socket_open():
                self.connect()
            ...
            if self.device_connected:
                ...
                print("---write successfully executed----")
            else:
        ...
        for block in addrmap:
            ...
            while remaining_length > MAX_LENGTH:
                ...
                if reg_type in ["ir", "hr"]:
                    if len(val.registers) < MAX_LENGTH:
                        raise ModbusIOException(f"Short read at {start_addr}: Expected {MAX_LENGTH}, got {len(val.registers)}")
                    data[-1] += val.registers
                elif reg_type in ["di", "co"]:
                    data[-1] += val.bits[:MAX_LENGTH]
            ...
            if reg_type in ["ir", "hr"]:
                if len(val.registers) < remaining_length:
                    raise ModbusIOException(f"Short read at {start_addr}: Expected {remaining_length}, got {len(val.registers)}")
                data[-1] += val.registers
            elif reg_type in ["di", "co"]:
                data[-1] += val.bits[:remaining_length]
