'''Deployed in Helical DG-PV Sync
Modbus RTU Write Error Fix
https://gemini.google.com/share/690ec309973b
'''
...
class modbusRTUDevice(ctrl.systemDevice):
    ...
    def connect(self):
        try:
            ...
            time.sleep(0.05)  # Yield strictly to let the OS clear previous locks
            self.device_connected = self.mbus_client.connect()
            if not self.device_connected:
