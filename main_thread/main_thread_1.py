...
# Ensure the filename is correct (e.g., fault_reporting.py)
...
if __name__ == "__main__":
    ...
    while(not install_file):
        readDeviceList()
        time.sleep(1) # Added a small sleep to prevent a tight loop if config is missing

    rpthndler.data_handler = rpthndler.dataBank()

    # Construct the path to the error codes file
    error_codes_path = os.path.join(path_config.path_cfg.base_path, 'modbus_mappings', 'error_codes.json')
    # Instantiate the Fault Processor
    fault_processor = FaultProcessor(error_codes_path=error_codes_path, poll_interval=10) # Poll every 10 seconds

    # Define all threads
    t1 = threading.Thread(target=getData)
    t2 = threading.Thread(target=rpthndler.data_handler.runDataLoop)
    # Add the new thread for fault processing
    ...
    print("All threads started: Data Acquisition, Report Handling, and Fault Processing.")
