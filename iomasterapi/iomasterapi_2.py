...
class ioDevice(ctrl.systemDevice):
            ...
            for part_num, device_info in devices.items():
                ...
                for pin_name, pin_info in device_info[io_key].items():
                    if isinstance(pin_info, dict) and 'address' in pin_info:
                        mcp_addresses.add(pin_info['address'])
        
        for address in mcp_addresses:
            try:
                addr_int = int(address, 16)
                mcp = MCP23017(self.i2c, address=addr_int)
                self.mcp_devices[address] = mcp
                logging.info(f"Initialized MCP23017 at address {address}")
            except Exception as e:
                logging.error(f"Failed to initialize MCP23017 at address {address}: {e}")
        ...
        for part_num, device_info in digital_inputs.items():
            for pin_name, pin_info in device_info['digital_input'].items():
                if isinstance(pin_info, dict) and 'address' in pin_info and 'pin_num' in pin_info:
                    ...
                    if address in self.mcp_devices and 0 <= pin_num <= 15:
                        try:
                            pin = self.mcp_devices[address].get_pin(pin_num)
                            pin.direction = digitalio.Direction.INPUT
                            pin.pull = digitalio.Pull.UP
                            
                            pin_key = f"in_{address}_{pin_num}"
                            self.mcp_pins[pin_key] = pin
                            logging.info(f"Set up MCP23017 input pin {pin_num} at address {address} for {part_num}:{pin_name}")
                        except Exception as e:
                            logging.error(f"Failed to set up MCP23017 input pin {pin_num} at address {address}: {e}")
                ...
                if isinstance(pin_info, dict) and 'address' in pin_info and 'pin_num' in pin_info:
                    ...
                    if address in self.mcp_devices and 0 <= pin_num <= 15:
                        try:
                            ...
                            self.mcp_pins[pin_key] = pin
                            logging.info(f"Set up MCP23017 output pin {pin_num} at address {address} for {part_num}:{pin_name}")
                        except Exception as e:
                            logging.error(f"Failed to set up MCP23017 output pin {pin_num} at address {address}: {e}")
            ...
            for pin_name, pin_info in device_info['analog_input'].items():
                if isinstance(pin_info, dict) and 'address' in pin_info and 'pin_num' in pin_info:
                    ...
                    if address in self.ads_devices:
                        try:
                            chan = AnalogIn(self.ads_devices[address], chan_enum)
            ...
            for pin_name, pin_info in device_info['digital_input'].items():
                try:
                    if isinstance(pin_info, int):
                        ...
                        if pin_key in self.mcp_pins:
                            state = not self.mcp_pins[pin_key].value
                            data[part_num][pin_name] = state
                        else:
    ...
    def write_digital_output(self, part_num, pin_name, state):
        ...
        if part_num not in digital_outputs or pin_name not in digital_outputs[part_num]['digital_output']:
            return False
        ...
        try:
            if isinstance(pin_info, int):
                GPIO.output(pin_info, GPIO.HIGH if state else GPIO.LOW)
                return True
                ...
                if pin_key in self.mcp_pins:
                    self.mcp_pins[pin_key].value = bool(state)
                    return True
                return False
            return False
        except Exception as e:
            logging.error(f"Error setting output {part_num}:{pin_name}: {e}")
            return False
