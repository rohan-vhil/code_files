'''Of course. I've updated the fault_reporting.py script to remove all the extra diagnostic print statements, leaving only the two you requested.

The code now only prints the raw data it receives from ctrl.getFaultData() and the final JSON payload right before it's sent to the API.'''
...
--- FaultProcessor: Received data from getFaultData(): {'solar-inverter:MAC_100KTL:456': {'fault': 0}} ---
--- FaultProcessor: PAYLOAD TO BE SENT:
[
    {
        "timestamp": 1759329235,
        "solar-inverter:MAC_100KTL:456": {
            "fault_code": "0",
            "fault_message": "Fault Cleared",
            "severity": "Info"
