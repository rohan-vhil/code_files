...
class FaultProcessor:
    ...
    def _send_faults_to_api(self, payload: dict):
        # The payload is now a single dictionary, not a list
        if not payload:
            return
        try:
            response = requests.post(self.api_url, json=payload, timeout=10)
            response.raise_for_status()
            # The number of devices with new faults is len(payload) - 1 (to exclude the timestamp key)
    ...
    def run(self):
        ...
        while self._running:
            try:
                ...
                # This will hold the new faults, keyed by device_id
                ...
                # If there are any new faults, add a timestamp and send the single object
