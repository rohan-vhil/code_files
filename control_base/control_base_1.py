'''The measuredData class has been modified to no longer
pre-initialize all possible parameters.
Instead, attributes are now created dynamically within
the createMeasureRegisterMap method of the systemDevice class
only if they are present in the mappings.json for that specific device.

only measuredData, createMeasureRegisterMap & getAllData changed
    ...
    def encodeWrite(self, msg_json:dict):
        if msg_json["param"] == "active_power":
            if(self.control_data.poweer_lt.model_present):
                ...
                self.control_data.power_pct_stpt.value = eval(msg_json['value'])*100/self.rated_power
