'''The issue is in your getAllData function. When the newly read total_energy is less than the last_valid_total_energy, your code doesn't add the total_energy key to the output dictionary at all. This results in it being missing, and whatever processes the data afterward likely interprets the missing value as zero.

The fix is to add an else block to assign the last known valid value to the output when a lower, invalid value is detected. This ensures the energy reading never goes backward.

I have updated the code to correct this behavior and to log these invalid readings to a file named total_energy_issues.json for your review, as you requested.

New prompt control_base *control_base_8.py is continouts prompted from chat
*Code update for data reading*
added to afpro on 22nd August 2025
'''
...
TOTAL_ENERGY_LOG_PATH = os.path.join(path_config.path_cfg.base_path, 'total_energy_issues.json')
            ...
            if hasattr(device.measured_data, param):
                ...
                if model is not None:
                    if param == 'total_energy':
                        ...
                        if current_energy >= device.last_valid_total_energy:
                            data[device_id_str][param] = current_energy
                            device.last_valid_total_energy = current_energy
                        else:
                            data[device_id_str][param] = device.last_valid_total_energy
                            log_entry = {
                                "timestamp": datetime.datetime.now().isoformat(),
                                "device_id": device_id_str,
                                "event": "Invalid total_energy reading detected (value was less than previous).",
                                "invalid_new_value": current_energy,
                                "last_valid_value_used": device.last_valid_total_energy
                            }
                            try:
                                with open(TOTAL_ENERGY_LOG_PATH, 'a') as log_file:
                                    log_file.write(json.dumps(log_entry) + '\n')
                            except Exception as e:
                                print(f"Error writing to total_energy log: {e}")
                    else:
