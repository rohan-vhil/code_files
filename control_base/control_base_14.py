'''Of course. I've updated the script to make the total_energy reading persistent across restarts and to handle the validation logic you described.

A new file, total_energy_log.json, will now be created and used as the single source of truth for the last known valid energy reading for each device.

Here are the key changes:

A new ENERGY_LOG_PATH variable has been added.

The systemDevice class no longer stores last_valid_total_energy in memory.

The getAllData function now reads from and writes to total_energy_log.json to:

Load the last valid energy reading when the script starts.

If a new reading is lower than the logged value or jumps by more than 100, it uses the logged value.

If the new reading is valid, it uses that value and updates the log file for the next run.'''
