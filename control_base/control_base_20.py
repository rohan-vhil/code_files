...
class operatingDetails:
    ...
    def time_of_use_func(self):
        ...
        if abs(price_delta_pct) < 0.05:
            ...
            sensitivity = 5.0
            raw_stpt = price_delta_pct * sensitivity * 100 * self.ref
            self.storage_stpt = max(-100, min(100, raw_stpt))
        if self.storage_stpt > 0 and self.battery_soc <= 20:
            self.storage_stpt = 0
        elif self.storage_stpt < 0 and self.battery_soc >= 95:
            self.storage_stpt = 0
        print(f"ToU -> Price: {current_price:.2f}, Avg: {avg_cost:.2f}, SoC: {self.battery_soc}%, Stpt: {self.storage_stpt:.2f}%")
