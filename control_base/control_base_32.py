'''Debugging Modbus Data Decoding Errors
Deployed on Helical controlling DG-PV Sync
'''
...
# Constants for Closed-Loop PF Control
PF_TARGET = 0.95
TAN_PHI_TARGET = 0.328  # Calculated as tan(acos(0.95))
...
class controlData:
    ...
    reactive_stpt: dataModel  # Added for SG125CX-P2 PF Control
    ...
    def __init__(self) -> None:
        ...
        self.reactive_stpt = dataModel("reactive_stpt", scaleData, 1) # Initialization for PF Control
        pass
...
class systemDevice:
        ...
        for batch in self.addr_map["map"]:
            ...
            for param_name in map_data:
                if self._process_list_parameter(param_name, batch, i, staged_phase_data, staged_component_data):
                    continue
                if param_name in agg_data or param_name in fault_data or param_name in device_status_data:
    ...
    def encodeWrite(self, msg_json:dict):
        ...
        if msg_json["param"] == "active_power":
            ...
            if self.control_data.reactive_stpt.model_present:
                ...
                # Step 1: Set Mode to 0xA2 (Reactive Power Adjustment Valid)
                # if self.control_data.reactive_stpt.has_mode:
                #     decoded = BinaryPayloadBuilder(byteorder=Endian.BIG, wordorder=Endian.LITTLE)
                #     decoded.add_16bit_uint(0xA2)
                #     payload = decoded.build()
                #     self.writeDataToRegisters(mbus.bytes_to_registers(payload), self.control_data.reactive_stpt.mode_start_addr + self.control_data.reactive_stpt.mode_offset)
                #     time.sleep(0.2)

                # Step 2: Write kVar value (scaled by 0.1)
                decoded = BinaryPayloadBuilder()
                decoded.add_16bit_int(self.control_data.reactive_stpt.value)
                payloaded=decoded.build()
                print("payload of stpt is ",payloaded)
                self.writeDataToRegisters(mbus.bytes_to_registers(payloaded), self.control_data.reactive_stpt.batch_start_addr + self.control_data.reactive_stpt.offset)
                decoded = BinaryPayloadBuilder()
                decoded.add_16bit_uint(0xA2)
                payload=decoded.build()
    ...
    def decodeData(self, data_set):
        ...
        #here or at getAllData we have to calculate aveage/means values for empty data
...
class operatingDetails:
    ...
    aggBatt = 0
    aggGrid = 0
    aggGrid_Q = 0 # Track net reactive power at grid meter
    aggLoad = 0
    aggEV = 0
    load = 0
    Ki = 0
    ...
    def controlGridPF(self):
        # Target Q for 0.95 PF based on active power load at the grid
        target_q_at_meter = abs(self.aggLoad) * TAN_PHI_TARGET
        
        # Error between target Q and current Grid Q
        q_error = target_q_at_meter - self.aggGrid_Q
        ...
        for device in device_list:
            if device.device_type == deviceType.solar:
                ...
                if hasattr(device.measured_data, 'reactive_power') and device.measured_data.reactive_power.model_present:
                    current_inv_q = device.measured_data.reactive_power.value
                    
                # Calculate new setpoint in 0.1kVar units
                new_q_stpt = (current_inv_q + q_error) / 100.0
                
                # Clamp to SG125CX-P2 capability: +/- 75.0 kVar (Value 750)
                new_q_stpt = max(min(new_q_stpt, 750), -750)
                new_q_stpt = 200
    ...
    def dg_pv_sync_func(self):
        ...
        print(f"margin : {margin} , ramp_up : {ramp_up} , ramp_down : {ramp_down} , deadband : {deadband}")
        if not hasattr(self, "pv_stpt"):
            self.pv_stpt = 0

        if self.aggDG <= self.dg_lim:
            print(f"aggDG is {self.aggDG} <= dg_limit {self.dg_lim} setting pv_stpt as 0")
            self.pv_stpt = 0
            return
        ...
        print(f"diff is {pv_allowed} - {self.pv_stpt} : {diff}")

        if abs(diff) > deadband:
            if diff > 0:
                self.pv_stpt = min(self.pv_stpt + ramp_up, pv_allowed)
                print(f"final stpt going to write : {self.pv_stpt}")
            else:
...
def getAggGrid():
    ...
    system_operating_details.aggGrid_Q = 0 # Track net reactive power
    for device in device_list:
...
def runSysControlLoop():
    ...
    # Run the Reactive Power Control Cycle
