...
https://gemini.google.com/share/1e515b51b303

I have extracted the complex parsing logic into a helper
method _process_list_parameter and added a _finalize_staged_lists method.
This makes createMeasureRegisterMap significantly cleaner and easier
to read while maintaining the robust "Staging" approach.'''
...
class systemDevice:
    ...
    def _process_list_parameter(self, param_name, batch, block_num, staged_phase, staged_component):
        """Helper to parse list-based parameters (L1_.., string_1_..) and add to staging dicts."""
        
        # 1. Check Phase Data (L1_voltage, etc.)
        ...
        # 2. Check Component Data (string_1_current, etc.)
    ...
    def _finalize_staged_lists(self, staged_data, target_attr_map=None):
        """Converts staged dictionary {idx: model} to sorted lists [model, None, model] and assigns to measured_data."""
    ...
    def createMeasureRegisterMap(self):
        ...
        for batch in self.addr_map["map"]:
            ...
            for param_name in map_data:
                # Try to process as list/indexed parameter
                if self._process_list_parameter(param_name, batch, i, staged_phase_data, staged_component_data):
                    continue
                
                # Process Regular Scalars (Aggregates & Faults)
                if param_name in agg_data or param_name in fault_data:
