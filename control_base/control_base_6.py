'''Based on the symptoms, the issue is that your code was not processing the raw Modbus data for your component-level parameters (MPPT and string). The main decodeData function was missing the necessary loops to handle them, so their values were never updated from the default of 0.

I have updated the decodeData function in your main script to correctly process this data. Your second script for averaging and sending data does not need any changes.
            ...
            if hasattr(device.measured_data, base_param):
                ...
                for model in models:
                    if model is not None and model.model_present:
                        ...
                        data[device_id_str][category][output_name] = model.value
                    idx += 1
    return data
