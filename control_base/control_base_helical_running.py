...
agg_data = ['power_factor','total_power','total_energy','today_energy','total_voltage','acfreq','temperature','apparent_power','reactive_power','input_power','maximum_discharging_power','maximum_cell_voltage','cell_number_with_maximum_voltage','minimum_cell_voltage','wind_direction','wind_speed','humidity','solar_radition','rain_gauge','global_horizontal_irradiance','global_tilt_irradiance']
data_decode = {
    "V" : "voltage",
    "I" : "current",
    "P" : "power",
...
class operatingDetails:
    ...
    def controlGridPF(self):
        ...
        if system_operating_details.aggDG > 0:
            is_dg_on = True
            print(f"dg is on and power of dg is {system_operating_details.aggDG}, turning off controlling reactive power ")
        else:
            print("DG is off")
            is_dg_on = False
    ...
    def dg_pv_sync_func(self):
        ...
        if self.grid_state != gridState.off:
            ...
            safe_delay = 720 # Set your DG cool-down/transfer delay in seconds
            ...
            if elapsed_time > safe_delay and self.aggDG == 0:
...
def getActiveControlMode():
    ...
    # FORCING DG PV SYNC CONTINUOUSLY
    # Removed the "if aggDG > 0" check so that the DG cool-down logic can run uninterrupted
    ...
    try:
        with(open(CONTROL_JSON_PATH) as control_file):
            ...
            # We skip reading 'mode' from json since it's forced above, 
            # but we still safely pull constraints like limits.
...
def runSysControlLoop():
    ...
    if system_operating_details.aggDG > 10:
        ...
        # If the DG is off, we assume the grid is connected.
