'''The AttributeError occurs because the code attempts to access
storage_capacity on every device type within the getAgg function,
but that attribute is only relevant for batteries.

I've resolved this by:

Ensuring the storage_capacity attribute is initialized on every systemDevice object.

Updating the getAgg function to only aggregate storage_capacity for battery devices.
    ...
    def createMeasureRegisterMap(self):
        ...
        for batch in self.addr_map["map"]:
            ...
            for param_name in map_data:
                ...
                if not is_phase_data and param_name in agg_data:
    ...
    def encodeWrite(self, msg_json:dict):
        if msg_json["param"] == "active_power":
            if(self.control_data.poweer_lt.model_present):
                ...
                print("-------into power_pct_stpt model-------")
                logging.info("-------into power_pct_stpt model-------")
                
                self.control_data.power_pct_stpt.value = int(eval(msg_json['value'])*100/self.rated_power)
                print("pct_stpt in encodeWrite : ",self.control_data.power_pct_stpt.value)
                if self.control_data.power_pct_stpt.value == 0:
                    self.control_data.power_pct_stpt.value = 1
                    print("updated pct_stpt in encodeWrite : ",self.control_data.power_pct_stpt.value)
