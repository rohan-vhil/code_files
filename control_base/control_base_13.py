            ...
            if hasattr(device.measured_data, param):
                ...
                if model is not None:
                    if param == 'total_energy':
                        ...
                        # Check for validity: not less than previous AND the jump is not too large.
                        # The check for last_valid_total_energy == 0 handles the very first reading.
                        if current_energy >= device.last_valid_total_energy and \
                           (device.last_valid_total_energy == 0 or (current_energy - device.last_valid_total_energy) <= 100):
                            # Value is valid, use it and update the last valid value.
                            data[device_id_str][param] = current_energy
                            device.last_valid_total_energy = current_energy
                        else:
                            # Value is invalid (either a decrease or a large jump), use the last known good value.
