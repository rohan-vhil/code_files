'''
Python Code Fix for Data Mapping
https://gemini.google.com/share/b60359963aac

Yes, strictly relying on self.num_phases for list sizing when it defaults to 1 (but your map contains L3) will cause the data to be ignored or cut off.
To fix the aliasing issue and ensure you don't lose data even if num_phases is set to 1,
I have updated the logic to dynamically size the lists based on the highest index found in your mapping file (e.g., if it sees L3, it makes the list length 3 automatically).'''
...
class systemDevice:
    ...
    def createMeasureRegisterMap(self):
                ...
                if param_name.startswith('L') and '_' in param_name:
                    try:
                        ...
                        if prefix[1:].isdigit():
                            ...
                            # Logic changed: Use dictionary key to enforce index position, ignoring self.num_phases limit
                            if type_short:
                        ...
                        if param_name.startswith(prefix) and param_name.endswith(suffix) and param_name != base_param:
                            try:
                                ...
                                if num_str.isdigit():
                                    ...
                                    model = dataModel()
                                    self.createMapForVar(model, batch, i, param_name)
                                    # Logic changed: Use dictionary key to enforce index position
        ...
        # Reconstruction: Build list based on max found index (prevents aliasing and ignores num_phases limit)
