'''
This approach ensures that even if your script restarts,
it will not accept an erroneously low total_energy value
(e.g., after a device power cycle). It will instead continue reporting
the last valid reading from the log file until the device's counter surpasses it.
The structure of your code has been maintained.

New prompt control_base *control_base_10.py is continouts prompted from chat
*Code update for data reading*
added to afpro on 25th August 2025
added to asawa on 29th August 2025 removed from asawa as it is logging the bigger garbage value and then
avoid the real value of total energy'''
...
def load_energy_log():
    ...
    try:
        ...
        energy_log = {}
        logging.warning("total_energy_log.json not found or invalid. Starting with a new log.")

def save_energy_log():
    try:
        with open(ENERGY_LOG_PATH, 'w') as f:
            json.dump(energy_log, f, indent=4)
    except IOError as e:
        logging.error(f"Could not save total_energy_log.json: {e}")
...
class systemDevice:
    ...
    connected_to : str = ""
    minimum_limit : int = 0
    last_valid_total_energy: float = 0
    ...
    def __init__(self, devicetype, commtype, cfg,rated_power=3800,storage_capacity=0) -> None:
        ...
        self.last_valid_total_energy = 0
...
def getAllData():
    data = {}
    energy_log_updated = False
    for device in device_list:
        device_id_str = str(device.device_id)
        data[device_id_str] = {}

        if device.last_valid_total_energy == 0:
            device.last_valid_total_energy = energy_log.get(device_id_str, 0)
            ...
            if hasattr(device.measured_data, param):
                ...
                if model is not None:
                    if param == 'total_energy':
                        ...
                        last_known_energy = device.last_valid_total_energy
                        
                        if current_energy >= last_known_energy:
                            data[device_id_str][param] = current_energy
                            device.last_valid_total_energy = current_energy
                            if energy_log.get(device_id_str) != current_energy:
                                energy_log[device_id_str] = current_energy
                                energy_log_updated = True
                        else:
                            logging.warning(
                                f"Device {device.device_id}: total_energy dropped from "
                                f"{last_known_energy} to {current_energy}. "
                                "Using last valid value."
                            )
                            data[device_id_str][param] = last_known_energy
                    else:
...
# Load the persisted energy log when the script starts
load_energy_log()