...
https://gemini.google.com/share/5cbb0c71c78e
getCoilDiscreteStatusData()

I have updated control_base.py to:
Map coil_status_data and discrete_input_data during the measurement map creation.
Properly extract values from bitwise Modbus responses.
Include these data points in the getAllData dictionary.
Provide the requested getCoilDiscreteStatusData function for targeted status retrieval.

Mappings.json for coil and discrete
{
  "IO_Module_01": {
    "block1": {
      "start_address": 0,
...
Output of getCoilDiscreteStatusData
{
    "101": {
        "breaker": 1,
        "relay": 0
...
def getAllData():
    ...
    for device in device_list:
        ...
        standard_lists = [agg_data, fault_data, coil_status_data, discrete_input_data]
        for data_list in standard_lists:
...
def getCoilDiscreteStatusData():
    ...
    for device in device_list:
        device_status = {}
        for param in (coil_status_data + discrete_input_data):
            if hasattr(device.measured_data, param):
                model = getattr(device.measured_data, param)
                if model is not None and model.model_present:
