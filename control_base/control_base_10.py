'''Based on the logs, the problem is a Modbus communication failure. The error message Unable to decode response at address 0 indicates that your script is failing to read any data from the device. This is why all the values, including total_energy, are defaulting to zero.

The previous logic for total_energy allowed this zero value to be included in the JSON. The correct fix is to make the code more robust. It will now track the read status for each device. If a read fails, that device will be skipped entirely for that cycle, preventing the reporting of misleading zero values.

I have updated your main script to implement this error handling. Your second script for averaging data remains correct and does not need any changes'''
