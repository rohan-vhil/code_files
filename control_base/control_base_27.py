'''
Python Control System Code Analysis
https://gemini.google.com/share/f457db62eaff

Calculating all controlling functions setpoints in value

1. Where I Made the Changes
I modified the code in two main areas to shift from Percentage (%) to Value (Watts):

A. Control Logic Functions (operatingDetails class) I removed the math that converted values to percentages (multiplication by 100 and division by rated power). The functions now return raw power values (Watts).

controlFuncConstPower: Removed * 100 / system_operating_details.agg_batt_rated.
controlPVChargeOnly: Removed * 100 / ....
controlFuncFullBackup: Changed hardcoded -100 to -system_operating_details.agg_batt_rated (Max Discharge Power).
controlFuncFullExport: Changed hardcoded 100 to system_operating_details.agg_batt_rated (Max Charge Power).
time_of_use_func: Removed the trailing * 100 / capacity logic.
export_limit_func & dg_pv_sync_func: Removed the percentage scaling logic.

B. Mode Initialization (getActiveControlMode function) When reading limits from control.json (like storage_max, storage_min), the code originally treated them as raw percentages (e.g., 100).

Change: I multiplied these limits by agg_batt_rated or agg_pv_rated so the system operates entirely in Watts internally.

2. Example Calculation: daily_peak_th_base_func
Let's walk through how this function calculates the setpoint in the new code and how that value is written to the device.

Scenario Parameters:
Battery Rated Power (agg_batt_rated): 10,000 W (10 kW)

Grid Reference Limit (ref): 5,000 W (The limit you want to stay under)

Current Grid Draw (aggGrid): 8,000 W (You are exceeding the limit)

Step 1: The Calculation (Inside daily_peak_th_base_func)
The code is:
...
Calculate Difference: abs(5000 - 8000) = 3,000 W

Apply Limit: min(10,000, 3,000) = 3,000 W (This ensures we don't request more than the battery can physically provide)

Determine Direction:

aggGrid > ref (8000 > 5000) is True (1).

(2 * 1 - 1) = 1 (Positive value means Discharge).

Final Setpoint (storage_stpt): 3,000 * 1 = 3,000

Result: The control loop has calculated that the battery needs to discharge 3,000 Watts to bring the grid down from 8kW to 5kW.

3. Writing to Register (The Conversion)
This is the most critical part. The control loop is now "speaking" in Watts (3000), but your Modbus device might still "listen" in Percent (%).

The Workflow:
Dispatch: In runSysControlLoop, the code creates a message:

Python
# power is 3000
data_msg = {"param": "active_power", "value": "3000"}
device.encodeWrite(data_msg)
The Translation (Inside encodeWrite): This function acts as the translator. It checks what kind of register the device has.

Case A: Device expects Percentage (power_pct_stpt) This is where the magic happens. The code I preserved in systemDevice.encodeWrite handles this:

Python
elif(self.control_data.power_pct_stpt.model_present):
    # Calculation: (Target Watts * 100) / Rated Watts
    self.control_data.power_pct_stpt.value = int(eval(msg_json['value']) * 100 / self.rated_power)
Calculation: (3000 * 100) / 10000

Math: 300000 / 10000 = 30

Value written to Register: 30

The device receives "30", which it interprets as 30% power (which is exactly 3000W).

Case B: Device expects Watts (poweer_lt) If the device is configured to accept direct power values:

Python
if(self.control_data.poweer_lt.model_present):
    self.control_data.poweer_lt.value = eval(msg_json['value'])
Value written to Register: 3000

The device receives "3000" directly.

Summary
Control Logic: Calculates 3,000 (Watts).

Driver (encodeWrite): Converts it to 30 (Percent).

Register Write: The value 30 is sent over Modbus.'''
