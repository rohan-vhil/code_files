'''To prevent incorrect total_energy readings from being sent, I'll add a validation check. The code will now store the last valid total_energy for each device and will only include the new value in the JSON if it's greater than or equal to the last one.

This change only affects your main data acquisition script. Your second script for averaging and sending data does not need any changes.

Here is the complete, updated code for the main script.'''
