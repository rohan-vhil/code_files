'''This version reverts the change that skipped the entire device on a read error.

Now, the code will always include the device's data (even if values are zero due to a communication issue) but will specifically omit the total_energy field if its value is less than the last known good reading.

The changes are only in your main data acquisition script. Your second script for averaging data does not need to be modified.'''
