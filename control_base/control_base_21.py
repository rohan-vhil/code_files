'''Control JSON For Time-of-Use
https://gemini.google.com/share/535053abf780

The time_of_use_func has been upgraded with:
Aggressive Scaling: Introduced a sensitivity factor so that the battery responds more decisively to price peaks/valleys rather than tiny 1% movements.
Deadband Logic: Prevents the battery from toggling or wasting energy when the price is within 5% of the daily average.
SoC Guardrails: Built-in protection to stop discharging at 20% SoC and stop charging at 95% SoC.
Updated getAgg: Now correctly pulls both SoC and SoH from your battery devices to update system_operating_details

added battery_storage_capacity parameter
control.json required for time_of_use
{
    "mode": "time_of_use",
    "op_details": {
        "ref": 1.0,
...
class controlData:
    ...
    def __init__(self) -> None:
        ...
        self.power_stpt = dataModel("power_stpt",scaleData,1)
        pass
...
class operatingDetails:
    ...
    def time_of_use_func(self):
        ...
        current_price = costs[index]
        price_delta_pct = (current_price - avg_cost) / avg_cost
        
        if abs(price_delta_pct) < 0.05:
            self.storage_stpt = 0
        else:
            alpha = self.ref * self.battery_storage_capacity / sum([abs(x - avg_cost) for x in costs])
            self.storage_stpt = alpha * (current_price - avg_cost) * 100 / self.battery_storage_capacity
            
            sensitivity = 5.0
            self.storage_stpt = max(-100, min(100, self.storage_stpt * sensitivity))

        if self.storage_stpt > 0 and self.battery_soc <= 20:
            self.storage_stpt = 0
        elif self.storage_stpt < 0 and self.battery_soc >= 95:
            self.storage_stpt = 0
        print(f"ToU -> Price: {current_price:.2f}, Avg: {avg_cost:.2f}, SoC: {self.battery_soc}%, Cap: {self.battery_storage_capacity}, Stpt: {self.storage_stpt:.2f}%")
    ...
    for device in device_list:
        if device.device_type == device_type:
            ...
            if device.device_type == deviceType.battery:
                ...
                if hasattr(device.measured_data, 'SoC'):
                    system_operating_details.battery_soc = device.measured_data.SoC.value
                if hasattr(device.measured_data, 'SoH'):
                    system_operating_details.battery_soh = device.measured_data.SoH.value
