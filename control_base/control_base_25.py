'''Solar Data Reporting and Offline Storage
https://gemini.google.com/share/39ace71df434

No Updation in Time_of_Use function

This ensures that $L3$ data is always labeled as L3 in your JSON
regardless of whether $L1$ or $L2$ are present.

This ensures that mppt6_power is always tied to the 6th position in your data structure,
regardless of whether MPPT 1 through 5 are defined.

this is my code named control_base.py

Now this has the serious issue



the issue is if I have L1_voltage and L3_voltage in my mappings.json but the mappings.json does not have L2_voltage then it assigns L3_voltage as L2_voltage and its value it is of L3_voltage only cause the register is of L3_voltage but it assigns it to L2_voltage this same problem is with component_data also it assigns mppt3_voltage as mppt2_voltage when mppt2_voltage is not there in mappings.json



I guess this problem occus in createMeasureRegisteMap where it in loop takes by ith value index fix this issue you can also change its logic to fix this issue but I want this issue to be resolved so do what is best



so fix this issue properly and I want this issue to be resolved so update the code to resolve the issue and remember only update the necessary part for resolving the issue and don't touch the rest of the code the rest of the code should remain as it is untouched



so update the code and give me all updated code without any comments'''
...
coil_status_data = ['breaker','relay']
discrete_input_data = ['switch', 'relay']
...
class dataModel:
    ...
    def getData(self, data) -> None:
        try:
            if self.decoderFunc in ['decode_bits', 'bits']:
    ...
    def createMeasureRegisterMap(self):
        ...
        for batch in self.addr_map["map"]:
            ...
            for param_name in map_data:
                ...
                if not is_processed:
                    target_list = None
                    if param_name in agg_data: target_list = agg_data
                    elif param_name in fault_data: target_list = fault_data
                    elif param_name in coil_status_data: target_list = coil_status_data
                    elif param_name in discrete_input_data: target_list = discrete_input_data

                    if target_list and not hasattr(self.measured_data, param_name):
                        model = dataModel()
                        self.createMapForVar(model, batch, i, param_name)
        ...
        for type_short, models in staged_phase_data.items():
            setattr(self.measured_data, type_short, models)
        
        for base_param, model_dict in staged_component_data.items():
            if model_dict:
                max_key = max(model_dict.keys())
                final_list = [None] * (max_key + 1)
                for k, v in model_dict.items(): final_list[k] = v
                setattr(self.measured_data, base_param, final_list)
        ...
        for x in per_phase_data:
            if hasattr(self.measured_data, x):
                for model_instance in getattr(self.measured_data, x):
                    if model_instance:
                        model_instance.data = data[model_instance.block_num]
                        model_instance.getData(data)
        lists_to_decode = [agg_data, fault_data, coil_status_data, discrete_input_data]
        for data_list in lists_to_decode:
            for x in data_list:
                if hasattr(self.measured_data, x):
            ...
            if hasattr(self.measured_data, x):
                for model_instance in getattr(self.measured_data, x):
                    if model_instance:
                        model_instance.data = data[model_instance.block_num]
                        model_instance.getData(data)
        if(self.control_data.poweer_lt.model_present): self.control_data.poweer_lt.getFactors(control_data)
        if(self.control_data.power_pct_stpt.model_present): self.control_data.power_pct_stpt.getFactors(control_data)
...
class operatingDetails:
    ...
    def controlFuncConstPower(self):
        self.storage_stpt = max(min((100 * (system_operating_details.aggLoad - system_operating_details.aggPV - system_operating_details.ref) / system_operating_details.agg_batt_rated),self.storage_max),self.storage_min)
    ...
    def controlFuncFullBackup(self):
        if(self.grid_state == gridState.on): self.storage_stpt = -100
    def controlFuncFullExport(self): self.storage_stpt = 100
    def controlFuncGenLimit(self): self.pv_stpt = self.ref
    ...
    def export_limit_func(self):
        ...
        if(self.storage_stpt <= -100 and self.limit_export):
    ...
    def safety_control_func(self):
        if self.safety_control_mode == 0: io.ioDevice.write_digital_outputs(0)
        elif self.safety_control_mode == 1: io.ioDevice.write_digital_outputs(1, self.io_output_data)

def setParameter(data_json):
    if("mode" in data_json.keys()): updateOperatingMode(data_json['mode'])
    if "param" in data_json.keys():
        if(data_json['param'] == "active_power"):
            for device in device_list:
                if(device.device_id == eval(data_json['device_id'])): device.encodeWrite(data_json)
    if "device_state" in data_json.keys():
        ...
        data_to_ctrl:dict = {"address": address,"format" : device.control_data.device_state.decoderFunc,"value" : data_json["device_state"],"wo" : device.control_data.device_state.wordorder,"bo":device.control_data.byteorder}
...
def updateOperatingMode(mode_str, ref=0):
    ...
    if mode_str == "net_zero": system_operating_details.controlFunc = (system_operating_details.controlFuncConstPower)
    elif mode_str == "pv_charge_only": system_operating_details.controlFunc = (system_operating_details.controlPVChargeOnly)
    elif mode_str == "max_export": system_operating_details.controlFunc = (system_operating_details.controlFuncFullExport)
    elif mode_str == "power_backup": system_operating_details.controlFunc = (system_operating_details.controlFuncFullBackup)
    elif mode_str == "gen_limit":
        system_operating_details.ref = ref
        system_operating_details.controlFunc = (system_operating_details.controlFuncGenLimit)
    elif mode_str == "none": system_operating_details.controlFunc = (system_operating_details.controlFuncNone)
    elif mode_str == "safety_control_mode": system_operating_details.controlFunc = system_operating_details.safety_control_func

def batteryCurveFunc():
    if system_operating_details.system_curtail_state < 1: return 100 - 200 * system_operating_details.system_curtail_state
    else: return -100
def PVCurveFunc():
    if system_operating_details.system_curtail_state < 1: return 100
    else: return (100 * (system_operating_details.full_pv_curtail - system_operating_details.system_curtail_state) / (system_operating_details.full_pv_curtail - 1))
def SetController(Kp,Ki,Ts):
    system_operating_details.Kp, system_operating_details.Ki, system_operating_details.Ts = Kp, Ki, Ts
def getAgg(device_type):
    tmp, agg_power, agg_capacity = 0, 0, 0
    for device in device_list:
        if device.device_type == device_type:
            if hasattr(device.measured_data, 'total_power'): tmp += device.measured_data.total_power.value
            agg_power += device.rated_power
            if device.device_type == deviceType.battery: agg_capacity += device.storage_capacity
    return (tmp, agg_power, agg_capacity) if device_type == deviceType.battery else (tmp, agg_power)
def curtailStateToStpt():
    for device in device_list:
        if device.stptCurve != None:
            device.control_data.setpt = device.stptCurve()
            device.writeDataToRegisters({"AC Active_powerSetPct": device.control_data.setpt})
def setModeSrc(src : str):
    if(src == "schedule"): system_operating_details.mode_src  = modeSrc.from_schedule
    elif(src == "direct"): system_operating_details.mode_src = modeSrc.direct_comm
def processMQTTMessage(message : str):
    with(open(CONTROL_JSON_PATH,'w') as control_file): control_file.write(message)
def startLiveData(): system_operating_details.live_data, system_operating_details.live_data_timer = True, 60
def stopLiveData(): system_operating_details.live_data, system_operating_details.live_data_timer = False, 0
        ...
        try:
            with(open(CONTROL_JSON_PATH) as control_file):
                ...
                if("mode" in control_json):
                    ...
                    system_operating_details.ref = control_json["op_details"].get("ref", 0)
                    system_operating_details.limit_export = control_json["op_details"].get("Limit_export", False)
                    system_operating_details.storage_min = control_json["op_details"].get("storage_min", -100)
                    system_operating_details.storage_max = control_json["op_details"].get("storage_max", 100)
                    system_operating_details.solar_max = control_json["op_details"].get("solar_max", 100)
                else: system_operating_details.controlFunc = None
        except: system_operating_details.controlFunc = None

def getAllData():
    data = {}
    try:
        with open(ENERGY_LOG_PATH, 'r') as f: energy_log = json.load(f)
    except: energy_log = {}
    for device in device_list:
        d_id = str(device.device_id)
        data[d_id] = {}
        if device.device_type in deviceType_e2s: data[d_id]["type"] = str(device.num_phases) + "ph_" + deviceType_e2s[device.device_type]
        for param in per_phase_data:
            if hasattr(device.measured_data, param):
                for i, x in enumerate(getattr(device.measured_data, param)):
                    if x: data[d_id]["L" + str(i + 1) + "_" + data_decode[param]] = x.value
            ...
            if hasattr(device.measured_data, param):
                ...
                if model:
                    if param == 'total_power':
                        val = model.value
                        if 'HT' in d_id and val < 0: val *= -1
                        data[d_id][param] = val
                    elif param == 'total_energy':
                        cur, log = model.value, energy_log.get(d_id, 0)
                        fin = cur if cur >= log and (log == 0 or (cur - log) <= 1000) else log
                        energy_log[d_id] = fin
                        if fin > 0: data[d_id][param] = fin
                    else: data[d_id][param] = model.value
        for base in component_data.keys():
            if hasattr(device.measured_data, base):
                models = getattr(device.measured_data, base)
                cat, m_type = base.split('_', 1)
                if cat not in data[d_id]: data[d_id][cat] = {}
                for idx, model in enumerate(models):
                    if model and model.model_present: data[d_id][cat][f"{cat}{idx+1}_{m_type}"] = round(model.value, 2)
    with open(ENERGY_LOG_PATH, 'w') as f: json.dump(energy_log, f)
    return data

def getLivePower():
    lp = {}
    for d in device_list:
        if hasattr(d.measured_data, 'total_power'):
            m = getattr(d.measured_data, 'total_power')
            if m and m.model_present: lp[str(d.device_id)] = m.value
    return lp
def getCoilDiscreteStatusData():
    so = {}
    for d in device_list:
        ds = {}
        for p in (coil_status_data + discrete_input_data):
            if hasattr(d.measured_data, p):
                m = getattr(d.measured_data, p)
                if m and m.model_present: ds[p] = m.value
        if ds: so[str(d.device_id)] = ds
    return so
def getFaultData():
    fo = {}
    for d in device_list:
        ...
        for p in fault_data:
            if hasattr(d.measured_data, p):
                m = getattr(d.measured_data, p)
                if m and m.model_present: df[p] = m.value
        if df: fo[str(d.device_id)] = df
    return fo
def getAggDG():
    system_operating_details.aggDG = 0
    for d in device_list:
        if(d.device_type == deviceType.meter and d.connected_to == deviceType.DG):
             if hasattr(d.measured_data, 'total_power'): system_operating_details.aggDG += d.measured_data.total_power.value
def getAggDGlim():
    system_operating_details.dg_lim = 0
    for d in device_list:
        if(d.device_type == deviceType.meter and d.connected_to == deviceType.DG): system_operating_details.dg_lim += d.minimum_limit
def getAggGrid():
    system_operating_details.aggGrid = 0
    for d in device_list:
        if(d.device_type == deviceType.meter and d.connected_to == deviceType.grid):
            if hasattr(d.measured_data, 'total_power'): system_operating_details.aggGrid += d.measured_data.total_power.value
def getAggLoad():
    system_operating_details.aggLoad = 0
    for d in device_list:
        if(d.device_type == deviceType.meter and d.connected_to == deviceType.load):
            if hasattr(d.measured_data, 'total_power'): system_operating_details.aggLoad += d.measured_data.total_power.value
        elif(d.device_type == deviceType.load and d.connected_to == deviceType.grid):
            if hasattr(d.measured_data, 'total_power'): system_operating_details.aggLoad += d.measured_data.total_power.value
def runSysControlLoop():
    ...
    getAggLoad(), getAggDGlim(), getAggDG(), getActiveControlMode()
    if system_operating_details.controlFunc != None:
        system_operating_details.controlFunc()
        for d in device_list:
            if d.device_type == deviceType.battery: d.encodeWrite({"param" : "active_power","value":str(system_operating_details.storage_stpt)})
            if d.device_type == deviceType.solar:
                d.control_data.power_pct_stpt.value = system_operating_details.pv_stpt
                d.encodeWrite({"param" : "active_power","value":str(d.control_data.power_pct_stpt.value)})
