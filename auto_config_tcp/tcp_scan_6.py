...
def read_modbus_register(client: ModbusTcpClient, slave_id: int, param_info: Dict, register_type: str) -> Optional[float]:
    try:
        ...
        if regs.isError():
            time.sleep(GATEWAY_RECOVERY_DELAY)
            return None 
...
def discover_active_slaves(client: ModbusTcpClient, slave_ids: List[int]) -> List[int]:
    active_slaves = []
    ...
    for slave_id in slave_ids:
        ...
        try:
            ...
            if regs.isError():
                if isinstance(regs, ExceptionResponse) and regs.exception_code in [1, 2, 3, 4]:
...
def detect_comm_details(ip: str, port: int, cfg: Dict, maps: Dict) -> List[ModbusDevice]:
    ...
    with client as conn:
        ...
        active_slaves = discover_active_slaves(conn, SLAVES)
