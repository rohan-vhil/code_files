...
def check_port(ip: str, port: int, timeout: float) -> Optional[str]:
    try:
...
def scan_subnet_for_port(subnet: str, port: int, timeout: float = 0.2) -> List[str]:
    active_ips = []
    network = ipaddress.IPv4Network(subnet, strict=False)
    with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
        futures = [executor.submit(check_port, str(ip), port, timeout) for ip in network.hosts()]
...
def detect_all_devices(subnets: List[str]) -> List[ModbusDevice]:
    ...
    for subnet in subnets:
        ...
        for port in TCP_PORTS:
            print(f"Discovering active hosts on port {port}...")
            active_ips = scan_subnet_for_port(subnet, port)
            
            if not active_ips:
                print(f"No active IPs found on port {port}.")
                continue
                
            print(f"Found active IPs: {active_ips}")
            for ip in active_ips:
                found_on_ip = detect_comm_details(ip, port, cfg, maps)
                all_devices_found.extend(found_on_ip)
